...
def train(model, optimizer, loader, loss_method, with_SAM, rank):
    model.train()
    loss_all = 0
    count = 0
    for data in loader:
        data = data.to(rank)
        output = model(data)
        loss_func = getattr(F, loss_method)
        if with_SAM:
            # First Step
            ...
            # Second Step
            loss_func(model(data), data.y).backward()
...
def trainer(
    ...
    for epoch in range(1, epochs + 1):
        ...
        ##Train model
        train_error = train(model, optimizer, train_loader, loss, with_SAM, rank=rank)
        if rank not in ("cpu", "cuda"):
...
def train_regular(
    ...
    base_optimizer = getattr(torch.optim, model_parameters["optimizer"])
    if job_parameters["with_SAM"]:
        optimizer = SAM(model.parameters(), base_optimizer, lr=model_parameters["lr"],
                                            **model_parameters["optimizer_args"]
        )
        scheduler = getattr(torch.optim.lr_scheduler, model_parameters["scheduler"])(
            optimizer.base_optimizer, **model_parameters["scheduler_args"])
    else:
